...
const getPriorityVariant = (
    priority: string,
): "active" | "inactive" | "warn" | "normal" => {
    switch (priority.toLowerCase()) {
        case "urgent": return "inactive";
...
const getStatusVariant = (
    status: string,
): "active" | "inactive" | "warn" | "normal" | "ghost" => {
    switch (status.toLowerCase()) {
        case "resolved":
        case "closed": return "active";
        case "cancelled": return "inactive";
        case "open":
        case "re_open": return "warn";
        case "in_progress":
        case "on_hold":
        default: return "normal";
    }
...
/* ════════════════════════════════════════════════════
   TICKET CARD
   ════════════════════════════════════════════════════ */
const TicketCard: React.FC<CommonCustomCardProps> = ({
    row,
    rowId,
    columns,
    onRowClick,
    ...
    // ── visible table fields only ──
    const title = getText(row.title);
    const status = getText(row.status);
    const priority = getText(row.priority);
    const plantName = getText(row.plant_name);
    const reporterName = getText(row.name);
    const email = getText(row.email);
    const phone = getText(row.phone_number);
    const componentType = getText(row.component_type);
    const component = getText(row.component_name);
    const assignedTo = getText(row.assignee_name);
    const dueDate = row.due_date ? formateDateTime(row.due_date) : "—";
        ...
        >
            ...
            <div className="relative px-3.5 pt-3.5 pb-3 overflow-hidden bg-[radial-gradient(circle_at_bottom_left,_#fff4eb_0%,_#fffaf5_40%,_#ffffff_100%)] dark:bg-[radial-gradient(circle_at_bottom_left,_#3b1f10_0%,_#26140a_32%,_#121212_100%)]">
                ...
                <div className="relative flex items-start gap-2.5">
                    ...
                    {onToggleSelect ? (
                        <button
                            ...
                            className={[
                                ...
                                selected ? "ring-2 ring-white/70 ring-offset-1 ring-offset-[#0f2a3b]" : "",
                            ].join(" ")}
                            style={{ width: 36, height: 36 }}
                        >
                            <span className={["absolute inset-0 transition-opacity duration-200", selected ? "opacity-0" : "opacity-100"].join(" ")}>
                                <Avatar label={reporterName} seed={rowId} size={36} />
                            </span>
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-0.5 flex items-center gap-1 text-[10px] text-black/50 dark:text-white/50 leading-none truncate">
                            <Ticket className="h-2.5 w-2.5 shrink-0" aria-hidden />
                            <span className="truncate">#{getText(row.ticket_number)} · {plantName}</span>
                        </p>
                    </div>
                ...
                <div className="relative mt-2.5 flex flex-wrap gap-1.5">
                    <Pill variant={statusVariant}>{status !== "—" ? status.replace(/_/g, " ") : "Unknown"}</Pill>
                    <Pill variant={priorityVariant}>{priority !== "—" ? priority : "—"}</Pill>
                    {componentType !== "—" && <Pill variant="normal">{componentType}</Pill>}
                </div>

                {/* ── Row 3: KPI chips ── */}
                <div className="relative mt-2 flex gap-1.5">
                    <KpiChip label="Reporter" value={reporterName} />
                    <KpiChip label="Assigned To" value={assignedTo} />
                    <KpiChip label="Due Date" value={dueDate} />
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-neutral-dark-100 px-3.5 py-3 grid grid-cols-2 gap-x-4 gap-y-2.5 border-t border-neutral-100 dark:border-white/5">
                <MetaField label="Email" value={email} />
                <MetaField label="Phone" value={phone} />
                <MetaField label="Component Type" value={componentType} />
                <MetaField label="Component" value={component} />
            </div>
        </div>
    );
};
