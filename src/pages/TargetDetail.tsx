...
const TargetDetails: React.FC = () => {
  ...
  const parameterEntries = Object.entries(target.parameters ?? {});
          ...
          {parameterEntries.length > 0 && (
            <DetailSectionCard>
              <DetailSectionHeader
                icon={BarChart2}
                title="Parameters"
                description="Parameter values and weekly history for this target"
              />
              <div className="p-2 space-y-1.5">
                {parameterEntries.map(([parameter, config]) => {
                  const historyEntries = Object.entries(config?.history ?? {});
                  return (
                    <div
                      key={parameter}
                      className="flex items-center gap-3 rounded-sm border border-neutral-200 dark:border-neutral-dark-200 bg-neutral-50 dark:bg-neutral-dark-50 px-3 py-2"
                    >
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-sm bg-brand-50 dark:bg-brand-900/20 border border-neutral-300 dark:border-neutral-dark-300">
                        <BarChart2 className="h-3.5 w-3.5 text-brand-500" />
                      </div>

                      <div className="min-w-0 shrink-0">
                        <p className="text-sm font-medium text-neutral-800 dark:text-neutral-dark-900 leading-tight">
                          {parameter}
                        </p>
                        <p className="font-mono text-xs text-neutral-400 dark:text-neutral-dark-400 leading-tight">
                      ...
                      {historyEntries.length > 0 && (
                        <div className="flex flex-1 flex-wrap items-center justify-end gap-1.5 min-w-0">
                          {historyEntries.map(([week, value]) => (
                            <span
                              key={week}
                              className="inline-flex items-center gap-1 rounded-full border border-neutral-200 dark:border-neutral-dark-200 bg-neutral-0 dark:bg-neutral-dark-100 px-2 py-0.5 text-[11px] leading-none whitespace-nowrap"
                            >
                              <span className="text-neutral-500 dark:text-neutral-dark-500">
                                {week}
                              </span>
                              <span className="font-mono font-medium text-neutral-800 dark:text-neutral-dark-900">
